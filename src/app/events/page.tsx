...
import { getEvents, getSponsors, Event, Sponsor } from '@/lib/api';
...
export default function EventsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setSponsors(sponsorsData?.slice(0, 3) || []);
      } catch (error) {
  ...
  const filteredEvents = events.filter((event) => {
    ...
    if (selectedFilter === 'PTA Meetings') return event.category?.toLowerCase().includes('meeting');
    if (selectedFilter === 'Fundraisers') return event.category?.toLowerCase().includes('fundraiser');
    if (selectedFilter === 'Social') return event.category?.toLowerCase().includes('social');
    return true;
  });
            ...
            <div className="bg-[#181411] text-white rounded-xl p-6">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-gray-400 text-sm text-center">
                  A huge thank you to our {new Date().getFullYear()}-{new Date().getFullYear() + 1} Platinum Sponsors for supporting our Wildcats!
                </p>
                <div className="flex flex-col gap-3 w-full">
                  {sponsors.length === 0 ? (
                    [1, 2, 3].map((i) => (
                      <div key={i} className="bg-white rounded-lg p-3 flex items-center justify-center h-12">
                        <span className="material-symbols-outlined text-gray-300">storefront</span>
                      </div>
                    ))
                  ) : (
                    sponsors.map((sponsor) => (
                      ...
                      >
                        {sponsor.logo ? (
                          <img src={sponsor.logo} alt={sponsor.name} className="max-h-6 max-w-full object-contain" />
                        ) : (
                          <span className="text-xs font-bold text-gray-600">{sponsor.name}</span>
                        )}
                      </a>
                    ))
                  )}
                </div>
                <Link href="/sponsors" className="text-primary text-sm font-bold hover:underline flex items-center gap-1">
